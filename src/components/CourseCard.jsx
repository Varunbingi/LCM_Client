...
const CourseCard=({data})=>{
    ...
    return(
        <div onClick={()=> navigate("/course/description",{state: {...data}})}
         className='text-white w-[22rem] h-[430px] shadow-lg rounded-lg cursor-pointer group bg-zinc-700 '>
            ...
            <div className='p-3 space-y-1 text-white'>
                ...
                <p className='font-semibold'>Instructor: :<span className='font-bold text-yellow-500'>{data?.createdBy}</span></p>
                <p className='font-semibold'>Total Lectures :<span className='font-bold text-yellow-500'>{data?.numbersOflectures}</span></p>
            </div>
        </div>
    )
}
